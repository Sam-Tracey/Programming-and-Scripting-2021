...
specific statistic – the P-value. When we calculate the P-value, a number less
than 0.05 indicates that the data does not follow the normal distribution while
a number \> 0.05 indicates that the data does follow the normal distribution.
[19]
...
From the output we can see that the distribution of the petal width data for the
Setosa species of Iris does not follow a normal distribution so we can further
investigate this in Python by plotting a distribution plot using the seaborn
library:
...
The Seaborn library has a fabulous function called the pairplot which generates
a similar, but in my opinion, superior graph using one line of code!
